...
export default async function PlayerPage({ params }) {
  ...
  return (
    <>
      ...
      {/* Knockout Run — shown FIRST once the legend reaches the Round of 32:
          the knockout journey is the "now", so it leads; the group games below
          are the qualifying record. Games render as cards from wc2026.knockout[]
          as they're recorded; empty ⇒ just the current-stage line + a link to
          the full projected path on /road-to-the-final (ready for the R32). */}
      {curIdx >= 1 && (
      ...
      {/* Group Stage — the qualifying record. Once the legend is through to the
          knockouts the group is history, so the heading drops "Schedule" (these
          are results now) and the Knockout Run section above leads. */}
      <section className="schedule-section">
        <div className="section-header">
        ...
        <div className="group-header">
          ...
          {player.wc2026.groupTable ? (
            <LiveGroupTable
              ...
              fallback={player.wc2026.groupTable}
            />
          ) : (
